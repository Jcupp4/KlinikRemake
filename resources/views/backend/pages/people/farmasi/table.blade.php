                        ...
                        <div id="kt_project_users_card_pane" class="tab-pane fade show active">
                                        ...
                                        <div class="card shadow-sm rounded-4 border-0 h-100 w-100"
                                            ...
                                            <div class="card-body d-flex flex-column align-items-center p-5">
                                                ...
                                                <div class="symbol symbol-80px mb-4">
                                                    <div class="symbol-label bg-light-success d-flex align-items-center justify-content-center rounded-circle shadow"
                                                ...
                                                <!-- Nama -->
                                                <a href="#"
                                                    class="fs-5 text-dark fw-bold mb-1 text-center text-hover-success">
                                                    {{ $farmasi->person->name ?? '-' }}
                                                </a>

                                                <!-- Username / Email -->
                                                <div class="fw-medium text-muted mb-2 text-center small">
                                                    {{ $farmasi->user->username ?? '-' }} |
                                                    {{ $farmasi->user->email ?? '-' }}
                                                </div>

                                                <!-- Telepon -->
                                                <div class="mb-3 text-center">
                                                    <span class="badge bg-light-success px-3 py-2 fw-medium">
                                                        <i class="bi bi-telephone-fill me-1"></i>
                                                        {{ $farmasi->person->telepon ?? '-' }}
                                                    </span>
                                                </div>

                                                <!-- Shift Kerja -->
                                                <div class="mb-3 text-center">
                                                    <span class="badge bg-light-info px-3 py-2 fw-medium">
                                                        <i class="bi bi-clock-fill me-1"></i>
                                                        Shift: {{ ucfirst($farmasi->shift) ?? '-' }}
                                                    </span>
                                                </div>

                                                <!-- Status -->
                                                <div class="mb-4 text-center">
                                                    @if ($farmasi->status === 'aktif')
                                                ...
                                                <div class="d-flex justify-content-center gap-2 mt-auto">
                                                    <a href="#"
                                                        class="btn btn-outline-success btn-sm fw-semibold shadow-sm">
                                                        <i class="bi bi-eye me-1"></i> View
                                                    </a>
                                                    <form action="{{ route('farmasi.delete', $farmasi->id) }}"
                                                        method="POST"
                                                        onsubmit="return confirm('Yakin hapus data farmasi ini?')"
                                                        class="d-inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                            ...
                            <div class="d-flex flex-stack flex-wrap pt-10 align-items-center">
                                <div class="fs-6 fw-semibold text-muted">
                                    Showing {{ $farmasis->firstItem() ?? 0 }} to {{ $farmasis->lastItem() ?? 0 }} of
                                    {{ $farmasis->total() ?? 0 }} entries
                                </div>
                                <ul class="pagination mb-0">
                                    <li class="page-item previous">
                                        <a href="{{ $farmasis->previousPageUrl() }}" class="page-link">
                                            <i class="bi bi-chevron-left"></i>
                                        </a>
                                    </li>
                                    @foreach ($farmasis->getUrlRange(1, $farmasis->lastPage()) as $page => $url)
                                        <li class="page-item {{ $farmasis->currentPage() == $page ? 'active' : '' }}">
                                            <a href="{{ $url }}" class="page-link">{{ $page }}</a>
                                        </li>
                                    @endforeach
                                    <li class="page-item next">
                                        <a href="{{ $farmasis->nextPageUrl() }}" class="page-link">
                                ...
                                <div class="modal-content rounded-3 shadow-lg">
                                    ...
                                    <div class="modal-header bg-primary text-white">
                                        <h5 class="modal-title" id="staticBackdropLabel">
                                            <i class="fas fa-user-nurse me-2"></i> Form Tambah Farmasi
                                        </h5>
